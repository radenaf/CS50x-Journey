(function () {
    ...
    function renderLinkOrPlaceholder(href, label, className) {
        if (!href || href === "#") {
            return `<span class="${className || "text-link"}" aria-disabled="true">${label}</span>`;
        }

        return `<a class="${className || "text-link"}" href="${href}">${label}</a>`;
    }
                    ...
                    <div>
                        ...
                        <ul class="footer-links">
                            ${(site.institutionalLinks || [])
                                .map(
                                    (link) => `
                                        <li>${renderLinkOrPlaceholder(link.href, link.placeholder || link.label, "text-link")}</li>
                                    `
                                )
                                .join("")}
                        </ul>
            ...
            .map(
                (item) => `
                    <article class="content-card publication-card">
                        ...
                        <p>${item.abstract}</p>
                        ${renderLinkOrPlaceholder(item.url, "External publication URL", "text-link")}
                    </article>
                `
            )
            .join("");
                ...
                (item) => `
                    <article class="content-card resource-card">
                        ...
                        <dl class="definition-grid compact-grid">
                            ...
                            <div><dt>Link</dt><dd>${renderLinkOrPlaceholder(item.href, "Open resource", "text-link")}</dd></div>
                        </dl>
                    </article>
                `
            )
    ...
    function renderInternshipCards(items) {
        const application = items?.application || {};
        const readingResources = Array.isArray(items?.readingResources) ? items.readingResources : [];
        ...
        if (application.title) {
            cards.push(`
                <article class="content-card">
                    <p class="card-eyebrow">Internship Application</p>
                    <h3>${application.title}</h3>
                    <p>${application.description}</p>
                    ${renderLinkOrPlaceholder(application.href, application.cta || "Learn more", "button button-secondary")}
                </article>
            `);
        }

        readingResources.forEach((item) => {
            cards.push(`
                <article class="content-card">
                    <p class="card-eyebrow">Internship Reading</p>
                    <h3>${item.title}</h3>
                    <p>${item.description}</p>
                    ${renderLinkOrPlaceholder(item.href, item.cta || "Open resource", "text-link")}
                </article>
            `);
        });
